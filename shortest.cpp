        ...
        if (superstep() == 1)
        {
            if (id == get_query()[1]) 
            {
                ...
                for (int i = 0; i < nqvalue().neighbors.size(); ++ i)
                {
                    ...
                    if (it != nqvalue().v_time[i].end())
                    {
                    	send_message(nqvalue().neighbors[i], make_pair(it->second, it->first+it->second));
                    }
                }
            }
            else
        ...
        else
        {
            for (int i = 0; i < messages.size(); ++ i)
            {
            	...
            	if (it == qvalue().s.end() || it->second > p.second)
            	{
            	    ...
            	    if (p.first < qvalue().arrivalTime)
            	    qvalue().arrivalTime = p.first;
            	    ...
            	    for (int j = 0; j < nqvalue().neighbors.size(); ++ j)
            	    {
            	    	...
            	    	send_message(nqvalue().neighbors[j], make_pair(p.first+it2->second, it2->first+it2->second));
