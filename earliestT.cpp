...
public:
    ...
    void broadcast()
    {
    	for (int i = 0; i < nqvalue().neighbors.size(); ++ i) 
           send_message(nqvalue().neighbors[i].first, 0);
    }
    virtual void compute(MessageContainer& messages)
    {
		if (superstep() == 1)
		{
			...
			{
				qvalue().vis = 1;
				send_message(nqvalue().toOriginalId, nqvalue().timestamp);
		   		broadcast();
			}
		...
		else
		{
			if (nqvalue().timestamp < 0) //virtual vertex
			{
				if (nqvalue().originalID == get_query()[0]) 
		        {
				    ...
				    if (mini < qvalue().arrivalTime) qvalue().arrivalTime = mini;
		        }
			}
			else if (messages.size() > 0 && qvalue().vis == 0)
			{
				qvalue().vis = 1;
				send_message(nqvalue().toOriginalId, nqvalue().timestamp);
				broadcast();
